...
- 饮食斜率：基础体重斜率为 `12 ml/kg`。当体重高于参考体重时，清淡多蔬果按 `10 ml/kg`、均衡日常按 `12 ml/kg`、偏咸外卖多按 `15 ml/kg` 修正额外体重，避免低于参考体重时出现反向排序。
...
基础体重修正 = (当前体重 - 参考体重) × 12
饮食斜率修正 = max(当前体重 - 参考体重, 0) × (饮食斜率 - 12)
体重修正 = 基础体重修正 + 饮食斜率修正
活动补充 = 久坐办公 0ml / 较少运动 200ml / 中度活动 400ml / 高强度 700ml
最终目标 = clamp(round(基础喝水量 + 体重修正 + 活动补充, 50ml), 1200ml, 3000ml)
```
