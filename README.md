...
- 体重基准：以 `30 ml/kg/day` 作为安静日常的起点，占最终基础估算的 `70%`。
- 性别参考：按成年人常见饮品摄入参考值修正，占最终基础估算的 `30%`。未指定 `2600 ml/day`、女性 `2200 ml/day`、男性 `3000 ml/day`。
- 活动补充：根据每日活动量增加饮水目标，活动越高补充越多。
- 饮食修正：清淡且多蔬果时下调 `150 ml`，均衡日常不调整，偏咸或外卖多时上调 `250 ml`。
- 结果取整：最终目标按 `50 ml` 取整，方便用户选择和记录。

公式概览：

```text
基础目标 = 体重 × 30 × 0.7 + 性别饮品参考 × 0.3
活动补充 = 久坐 0ml / 轻度 250ml / 中度 500ml / 高强度 800ml
饮食修正 = 清淡多蔬果 -150ml / 均衡 0ml / 偏咸外卖多 +250ml
最终目标 = clamp(round(基础目标 + 活动补充 + 饮食修正, 50ml), 1200ml, 4000ml)
```
